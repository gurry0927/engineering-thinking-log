...
- **功能項目**：
    ...
    - **跨工具自動化 (GAS + VBA)**：
        - 調取 Sheets 開案紀錄，由 GAS 自動填寫報價單並轉出 Excel。
        - **VBA 補充**：針對 Sheets 無法修改頁首、頁碼、邊界的問題，設計 Excel VBA 腳本進行二次處理，確保文件符合商務正式格式。
...
## 📈 演進與反思：早期資料庫思維的萌芽
回顧 P-01 的開發，雖然當時使用的是 Google Sheets 作為存儲媒介，但在架構上已展現出關鍵的**資料庫設計思維**：
1. **唯一識別碼 (Primary Key)**：利用「上傳時間戳」結合「LineUserId」生成流水號，確保每筆交易與文件的唯一性。
2. **資料關聯 (Relational Logic)**：透過 10 多張工作分頁的相互引用，模擬了多對一、一對多的關聯查詢邏輯。
3. **模組化組裝 (Modular Clauses)**：條文生成不再是簡單的文字替換，而是透過 Key 值關聯不同的「子模組」，這與後來的 AI 模組化設計理念不謀而合。

> **總結**：P-01 是我的「架構初啟蒙」。雖然技術棧相對基礎，但其核心的邏輯拆解力與資料流設計，為後續 P-04 的產品化開發奠定了堅實的基礎。
