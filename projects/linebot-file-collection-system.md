# LineBot 檔案搜集與驗證系統 (P-02) - 合約轉化器

## 📌 專案簡介
本專案為自動化流程的進階應用，將 Line 作為全方位的業務入口。核心流程包含：**Line 接收報價單 XLSX -> 自動驗證並歸檔 -> 業務從 Line 選取特定版本 -> 基於 Doc 佔位符模板生成正式合約書 -> 自動觸發文件寄送**。

## 🚀 專案演進紀錄 (Evolution History)

### Phase 1: 業務資料入口與歸檔 (2025-10-01 ~ Ongoing)
**核心目標**：建立 LineBot 作為業務流程的第一現場。
- **功能項目**：
    - **第一階段：議價紀錄歸檔**：LineBot 接收報價 Excel -> 上傳 Drive -> GAS 解析欄位 -> 寫入 Sheet 資料庫。
    - **第二階段：PDF 回簽搜集**：
        - 接收合約回簽 PDF。
        - 呼叫 PDF-to-Image API 將合約拆分為圖片供快速查閱。
        - 同步分發圖片至主管與業務信箱，並歸檔至 Drive。
    - **第三階段：自動合約生成 (Flagship Feature)**：
        - LineBot 輸入案號 -> 歷史紀錄檢索 (Top 5) -> 選擇案號。
        - **模組化法規條文提取**：從資料庫抽取公司資料與價金，自動組裝出正式合約書。
        - **一鍵推送**：生成後自動推送至 Line 並發送至業務部與開發者信箱。
        - `10`: 驗證通過。
        - `-1`: 格式/名稱錯誤（已刪除）。
    - **資料存續**：同時記錄 `UserId` 以保留個人化推播的擴充空間。

## ⚙️ 技術架構
- **Interface**: Line Messaging API (File Message Event)
- **Engine**: Google Apps Script (GAS)
- **Database**: Google Sheet (Metadata Tracking)
- **Storage**: Google Drive (Physical Files)

## 🧩 遇到的挑戰與解決方法

### 1. 消失的檔名 (The Missing Filename Case)
- **挑戰**：Line API 在 Webhook 事件中不直接傳送檔案名稱，導致無法在第一時間進行規則檢查。
- **解決**：重定義流程順序。程式先透過 `getMessageContent` 下載二進制流，存入 Drive 生成實體檔案後，再讀取該檔案的 Property (`getName()`) 進行判斷。若不合規則，則觸發自動刪除。

### 2. 精準回傳與隱私權限
- **挑戰**：在匿名程度較高的群組中，難以確保能私訊通知特定上傳者。
- **解決**：在資料表中強制收集 `GroupId` 作為首要通知通道，同時保留 `UserId`。

---
> **備註**：此專案僅描述邏輯架構與技術挑戰，不包含任何公司內部敏感數據或私有代碼。
